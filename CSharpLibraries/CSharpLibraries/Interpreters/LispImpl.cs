  ...
  public sealed partial class Lisp{
    ...
    private object Expand(object x, bool topLevel = false){
      ...
      if (op.Equals(SymQuote)){
        ...
        Require(l, l.Count == 3);
        var v = l[1];
        Require(l, v is Symbol, "can set! only a symbol");
        ...
        if (v is IList<object> lv && lv.Count != 0){
          ...
          Require(l, l.Count == 3);
          Require(l, v is Symbol, "can define only a symbol");
          var exp = Expand(l[2]);
          if (op.Equals(SymDefineMacro)){
            Require(l, topLevel, "define-macro only allowed at top level");
            var proc = Eval(exp, _globalEnv);
            Require(l, proc is Lambda, "macro must be a procedure");
        ...
        var vars = l[1];
        var body = l.Skip(2).ToList();
        Require(l, (vars is IList<object> list &&
    ...
    private static object Let(IList<object> args, Lisp interpreter){
      ...
      Require(x, bindings.Cast<List<object>>().All(b => b is{Count: 2} && b[0] is Symbol),
